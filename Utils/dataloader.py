...
class DataLoader:
    def preprocess_for_graphs(self, train_path, iter_id=0, is_ihdp=False):
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        train_arr = np.load(train_path)
    
        np_train_X = torch.tensor(train_arr['x'][:, :, iter_id], dtype=torch.float32).to(device)
        np_train_T = torch.tensor(train_arr['t'][:, iter_id], dtype=torch.float32).unsqueeze(1).to(device)
        np_train_yf = torch.tensor(train_arr['yf'][:, iter_id], dtype=torch.float32).unsqueeze(1).to(device)
    
        if is_ihdp:
            train_X = torch.cat((np_train_X, np_train_yf), dim=1)
        else:
            np_train_e = torch.tensor(train_arr['e'][:, iter_id], dtype=torch.float32).unsqueeze(1).to(device)
